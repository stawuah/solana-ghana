...
export function validate(body, rules) {
  ...
  for (const [field, rule] of Object.entries(rules)) {
    ...
    // Single-line fields reach email subject lines, where a stray newline
    // renders as a broken header. Collapse any run of whitespace to one space.
    const normalised = rule.multiline ? value : value.replace(/\s+/g, ' ')

    if (rule.max && normalised.length > rule.max) {
      errors[field] = `Must be ${rule.max} characters or fewer`
      continue
    }
    if (rule.email && !EMAIL.test(normalised)) {
      errors[field] = 'Enter a valid email address'
      continue
    }
    if (rule.oneOf && !rule.oneOf.includes(normalised)) {
...
export const RULES = {
  ...
  team: {
    ...
    contribution: { required: true, max: 4000, multiline: true },
  },
  ambassador: {
    name: { required: true, max: 120 },
    email: { required: true, email: true, max: 254 },
    ...
    plan: { required: true, max: 4000, multiline: true },
  },
}
